...
def rebound_match(codes, start=None, end=None, save_result=False, policy_args=None):
    """ 调用rebound策略
    """
    logger.info("%s called with arguments: len(codes)=%d, start=%s, end=%s, save_result=%s, policy_args=%s" % ('rebound_match', len(codes), start, end, save_result, policy_args))
    return dogen.rebound.match(codes, start=start, end=end, save_result=save_result, policy_args=policy_args)

@app.task
def dispatcher_of_rebound_match(codes=None, start=None, end=None, save_result=False, policy_args=None, slice=1000):
    """ rebound策略任务拆分
    ...
    ### 拆分任务, 聚合结果
    logger.info('%s called to dispatch %d codes into sub-task with slice=%d' % ('dispatcher_of_rebound_match', len(codes), slice))
    reply = []
    while True:
        tasks = len(reply)
        if (tasks * slice) >= len(codes):
...
def hl_uptrend_match(codes, start=None, end=None, save_result=False, policy_args=None):
    """ 调用rebound策略
    """
    logger.info("%s called with arguments: len(codes)=%d, start=%s, end=%s, save_result=%s, policy_args=%s" % ('hl_uptrend_match', len(codes), start, end, save_result, policy_args))
    return dogen.uptrend.match(codes, start=start, end=end, save_result=save_result, policy_args=policy_args)

@app.task
def dispatcher_of_uptrend_match(codes=None, start=None, end=None, save_result=False, policy_args=None, slice=1000):
    """ rebound策略任务拆分
    ...
    ### 拆分任务, 聚合结果
    logger.info('%s called to dispatch %d codes into sub-task with slice=%d' % ('dispatcher_of_uptrend_match', len(codes), slice))
    reply = []
    while True:
        tasks = len(reply)
        if (tasks * slice) >= len(codes):
...
@app.task
def rs_fallback_match(codes, start=None, end=None, save_result=False, policy_args=None):
    """ 调用rebound策略
    """
    logger.info("%s called with arguments: len(codes)=%d, start=%s, end=%s, save_result=%s, policy_args=%s" % ('rs_fallback_match', len(codes), start, end, save_result, policy_args))
    return dogen.rs_fallback.match(codes, start=start, end=end, save_result=save_result, policy_args=policy_args)

@app.task
def dispatcher_of_rs_fallback_match(codes=None, start=None, end=None, save_result=False, policy_args=None, slice=1000):
    """ rs_fallback策略任务拆分
    ...
    ### 拆分任务, 聚合结果
    logger.info('%s called to dispatch %d codes into sub-task with slice=%d' % ('dispatcher_of_rs_fallback_match', len(codes), slice))
    reply = []
    while True:
        tasks = len(reply)
        if (tasks * slice) >= len(codes):
            break
        reply.append(celery_dogen.rs_fallback_match.delay(codes[tasks*slice:(tasks+1)*slice], start=start, end=end, save_result=save_result, policy_args=policy_args))
