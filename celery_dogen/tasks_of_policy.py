...
@app.task
def up_mandarin_match(codes, start=None, end=None, save_result=False, policy_args=None):
    """ 调用up_mandarin策略
    """
    logger.info("%s called with arguments: len(codes)=%d, start=%s, end=%s, save_result=%s, policy_args=%s" % ('up_mandarin_match', len(codes), start, end, save_result, policy_args))
    return dogen.up_mandarin.match(codes, start=start, end=end, save_result=save_result, policy_args=policy_args)

@app.task
def dispatcher_of_up_mandarin_match(codes=None, start=None, end=None, save_result=False, policy_args=None, slice=1000):
    """ up_mandarin策略任务拆分
    ...
    ### 拆分任务, 聚合结果
    logger.info('%s called to dispatch %d codes into sub-task with slice=%d' % ('dispatcher_of_up_mandarin_match', len(codes), slice))
    reply = []
    while True:
        tasks = len(reply)
        if (tasks * slice) >= len(codes):
            break
        reply.append(celery_dogen.up_mandarin_match.delay(codes[tasks*slice:(tasks+1)*slice], start=start, end=end, save_result=save_result, policy_args=policy_args))

    ### 聚合子任务结果
    return dispatcher_poll_result(reply)

@app.task
def up_ma5upper_match(codes, start=None, end=None, save_result=False, policy_args=None):
    """ 调用up_ma5upper策略
    """
    logger.info("%s called with arguments: len(codes)=%d, start=%s, end=%s, save_result=%s, policy_args=%s" % ('up_ma5upper_match', len(codes), start, end, save_result, policy_args))
    return dogen.up_ma5upper.match(codes, start=start, end=end, save_result=save_result, policy_args=policy_args)

@app.task
def dispatcher_of_up_ma5upper_match(codes=None, start=None, end=None, save_result=False, policy_args=None, slice=1000):
    """ up_ma5upper策略任务拆分
    ...
    ### 拆分任务, 聚合结果
    logger.info('%s called to dispatch %d codes into sub-task with slice=%d' % ('dispatcher_of_up_ma5upper_match', len(codes), slice))
    reply = []
    while True:
        tasks = len(reply)
        if (tasks * slice) >= len(codes):
            break
        reply.append(celery_dogen.up_ma5upper_match.delay(codes[tasks*slice:(tasks+1)*slice], start=start, end=end, save_result=save_result, policy_args=policy_args))

    ### 聚合子任务结果
    return dispatcher_poll_result(reply)

@app.task
def up_matouch_match(codes, start=None, end=None, save_result=False, policy_args=None):
    """ 调用up_matouch策略
    """
    logger.info("%s called with arguments: len(codes)=%d, start=%s, end=%s, save_result=%s, policy_args=%s" % ('up_matouch_match', len(codes), start, end, save_result, policy_args))
    return dogen.up_matouch.match(codes, start=start, end=end, save_result=save_result, policy_args=policy_args)

@app.task
def dispatcher_of_up_matouch_match(codes=None, start=None, end=None, save_result=False, policy_args=None, slice=1000):
    """ up_matouch策略任务拆分
    ...
    ### 拆分任务, 聚合结果
    logger.info('%s called to dispatch %d codes into sub-task with slice=%d' % ('dispatcher_of_up_matouch_match', len(codes), slice))
    reply = []
    while True:
        tasks = len(reply)
        if (tasks * slice) >= len(codes):
            break
        reply.append(celery_dogen.up_matouch_match.delay(codes[tasks*slice:(tasks+1)*slice], start=start, end=end, save_result=save_result, policy_args=policy_args))
