...
def exclude_analyze(basic, kdata, pick_index, take_index, rise_range, policy_args):
    ...
    maxi_close   = __parse_policy_args(policy_args, MAXI_CLOSE)
    outstanding  = __parse_policy_args(policy_args, OUTSTANDING)
    [from_index, high_index, inc_close, get_lhigh, tmp_id] = rise_range
    ...
    for temp_index in range(from_index-1, high_index, -1):
        ...
        if kdata.iloc[temp_index][dogen.VOLUME] <= kdata.iloc[temp_index+1][dogen.VOLUME]:
            continue
        if dogen.caculate_incr_percentage(kdata.iloc[temp_index][dogen.P_OPEN], kdata.iloc[temp_index+1][dogen.P_CLOSE]) < 3:
            continue
    ...
    ### 特征五
    if dogen.caculate_incr_percentage(kdata.iloc[pick_index][dogen.P_CLOSE], kdata.iloc[from_index][dogen.P_CLOSE]) > 50:
...
def stock_analyze(basic, kdata, policy_args):    
    ...
    result[dogen.RST_COL_SCORE]       = score_analyze(basic, kdata, pick_index, take_index, rise_range, policy_args) # 打分
