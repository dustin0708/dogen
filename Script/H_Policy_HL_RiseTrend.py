...
def policy_analyze_HL_long_takeindex(basic, kdata, pick_index, dparam):
    ...
    if (take_index is not None) and (take_index >= 3):
...
def policy_analyze_HL_long_drop(basic, kdata, pick_index, dparam):
    ...
    ## 相对涨停价涨幅限制
    maxi_close = Comm.GetParamValue(dparam, Comm.PARAM_MAXI_HEAP_RISEUP, 12)
...
def policy_analyze_HL_middle(basic, kdata, pick_index, dparam):
    """3~5天涨停判断
    """
    return None
    
def policy_analyze_HL_short(basic, kdata, pick_index, dparam):
    """3天以内涨停判断
    """
    return None
...
def policy_analyze_stock(basic, kdata, dparam):
    ...
    if pick_index > 5:
        ...
        return policy_analyze_HL_middle(basic, kdata, pick_index, dparam)
    else:
        return policy_analyze_HL_short(basic, kdata, pick_index, dparam)
